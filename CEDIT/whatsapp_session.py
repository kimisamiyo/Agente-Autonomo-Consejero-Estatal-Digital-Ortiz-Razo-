"""
Sesiones WhatsApp: historial, modo mentor y freemium por número.
"""
import json
import os
import re
...
SESSIONS_FILE = os.path.join(os.path.dirname(__file__), ".cedit_whatsapp_sessions.json")
...
def conversation_id(wa_id: str) -> str:
    return f"wa_{normalize_wa_id(wa_id)}"
...
class WhatsAppSession:
    def __init__(self, wa_id: str):
        self.wa_id = normalize_wa_id(wa_id)
        self.conv_id = conversation_id(self.wa_id)
...
def get_whatsapp_session(wa_id: str) -> WhatsAppSession:
    return WhatsAppSession(wa_id)
