        ...
        <div className="shrink-0 w-full px-3 sm:px-5 py-2.5 border-b border-slate-200 dark:border-slate-700/80 bg-white dark:bg-[var(--cedit-surface)] z-10">
          {showDecisionNetwork && showMentorHeader && (
            <div className="cedit-seg lg:hidden max-w-[min(1440px,100%)] mx-auto mb-2">
              <button
                type="button"
                className={headerPanelOpen !== 'network' ? 'is-on' : ''}
                onClick={() => setHeaderPanelOpen((p) => (p === 'mentor' ? null : 'mentor'))}
              >
                {t('jewel.tabs.mentor')}
              </button>
              <button
                type="button"
                className={headerPanelOpen === 'network' ? 'is-on' : ''}
                onClick={() => setHeaderPanelOpen((p) => (p === 'network' ? null : 'network'))}
              >
          ...
          >
            {showDecisionNetwork && (
              <div
                className={`${showMentorHeader ? 'lg:flex-1 min-w-0' : 'w-full'} ${
                  showDecisionNetwork && showMentorHeader && headerPanelOpen === 'mentor'
                    ? 'hidden lg:block'
                    : ''
                }`}
              >
            ...
            {showMentorHeader && (
              <div
                ...
                className={`${showDecisionNetwork ? 'lg:flex-1 min-w-0 min-h-[56px]' : 'w-full'} ${
                  showDecisionNetwork && showMentorHeader && headerPanelOpen === 'network'
                    ? 'hidden lg:block'
                    : ''
                }`}
              >
